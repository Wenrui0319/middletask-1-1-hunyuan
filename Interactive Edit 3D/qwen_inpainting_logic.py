...
async def run_generation(prompt, neg_prompt, input_img, sampler, scheduler, steps, cfg, denoise, shift, strength):
    ...
    try:
        ...
        # 更新工作流参数
        # --- 动态修复工作流：将VAEEncode替换为VAEEncodeForInpaint ---
        # 这是解决整图模糊问题的关键。我们用正确的Inpainting编码器替换了通用的编码器。
        # prompt_workflow["90"]["class_type"] = "VAEEncodeForInpaint"
        # prompt_workflow["90"]["inputs"]["mask"] = ["110", 0] # 将蒙版连接到Inpainting编码器
        # prompt_workflow["90"]["inputs"]["grow_mask_by"] = 6 # 可选：稍微扩大蒙版以获得更好的边缘融合
        ...
        prompt_workflow["110"]["inputs"]["channel"] = "red" # 修复：上传的已经是单通道遮罩，使用'red'通道直接读取其灰度值
