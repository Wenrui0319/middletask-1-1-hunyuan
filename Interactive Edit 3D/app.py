...
def build_app(args):
    ...
    with gr.Blocks(
        ...
        # Hidden textboxes to receive file paths from HTML
        preview_path_input = gr.Textbox(
            label="File Path for Preview", visible=False, elem_id="preview_path_input"
        )
        dispatch_path_input = gr.Textbox(
            label="File Path to Dispatch", visible=False, elem_id="dispatch_path_input"
        )
            ...
            with gr.Column(scale=9):
                ...
                with gr.Tabs() as tabs_output:
                    ...
                    with gr.Tab("Gemini Image Edit", id="Gemini Image Edit"):
                        gemini_image_edit_input_image = (
                            gemini_image_edit_app.create_gemini_image_edit_ui()
                        )
                    with gr.Tab("Hunyuan3D", id="Hunyuan3D"):
                        hunyuan_hidden_receiver = hunyuan_logic.create_hunyuan_ui(
        ...
        def handle_preview(file_path):
            """
            Handles previewing the selected file.
            """
            if not file_path:
                return None, None
            ...
            return prefixed_path, preview_update

        def handle_dispatch(file_path, active_tab):
            """
            Handles dispatching the selected file to the active tab.
            """
            if not file_path:
                return gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(value="")
            ...
            (
                ...
                gemini_state_update,
                gemini_text_update,
                gemini_image_edit_update,
                hunyuan_update,
            ) = dispatched_updates

            return sam_update, qwen_edit_update, qwen_inp_update, gemini_state_update, gemini_text_update, gemini_image_edit_update, hunyuan_update, gr.update(value="")

        preview_path_input.change(
            fn=handle_preview,
            inputs=[preview_path_input],
            outputs=[selected_file_state, image],
        )

        dispatch_path_input.change(
            fn=handle_dispatch,
            inputs=[dispatch_path_input, active_tab_state],
            outputs=[
                sam_input_image,
                ...
                gemini_text_input,
                gemini_image_edit_input_image,
                hunyuan_hidden_receiver,
                dispatch_path_input, # Add self-reference to clear the textbox
            ],
        )
