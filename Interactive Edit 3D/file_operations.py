...
def dispatch_image(files, active_tab_name):
    ...
    # This order MUST be kept in sync with the `outputs` list of the click event.
    tab_order = ["SAM", "Qwen Edit", "Qwen Inpainting", "Gemini Chat", "Gemini Image Edit", "Hunyuan3D"]
    
    # Initialize a list of "no update" signals for all 7 output components
    outputs = [gr.update()] * 7
    ...
    try:
        ...
        if active_tab_name in tab_order:
            ...
            if active_tab_name == "SAM":
            ...
            elif active_tab_name == "Gemini Image Edit":
                outputs[5] = full_path
                ...
                outputs[target_index] = inpainting_data
                 ...
                 # Hunyuan3D is the last tab, so its output is at index 6
                outputs[6] = full_path
            else:
