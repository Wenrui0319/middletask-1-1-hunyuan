...
def create_hunyuan_ui(SUPPORTED_FORMATS, HTML_OUTPUT_PLACEHOLDER, tabs_output, caption, mv_image_front, mv_image_back, mv_image_left, mv_image_right, file_out, file_out2):
    with gr.Row(equal_height=True):
        ...
        def on_image_path_received(filepath):
            ...
            try:
                ...
                gr.Error(f"从路径加载Hunyuan图片失败: {e}")
                return None, None

        hunyuan_hidden_receiver.change(
            fn=on_image_path_received,
            inputs=[hunyuan_hidden_receiver],
            outputs=[hunyuan_image_state, geneting_image]
        )
        
        btn.click(
            shape_generation,
            inputs=[caption, hunyuan_image_state, mv_image_front, mv_image_back, mv_image_left, mv_image_right, num_steps, cfg_scale, seed, octree_resolution, check_box_rembg, num_chunks, randomize_seed],
    ...
    return hunyuan_hidden_receiver