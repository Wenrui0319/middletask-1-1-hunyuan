...
try:
    #key1 AIzaSyBlFbwOK5xbV48YVaGEXBxObXioRJznpLE
    #key2 AIzaSyCqVsEkGfQ3rqdote_eMkJHZNLdWKgaRjI
...
def create_gemini_image_edit_ui():
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        gemini_image_edit_state = gr.State()

        with gr.Row():
            with gr.Column():
                visible_display_img = gr.Image(label="Input Image", type="pil", image_mode="RGBA", interactive=False)
                gemini_hidden_receiver = gr.Image(type="filepath", visible=False)
        ...
        def on_image_path_received(filepath):
            if not filepath:
                return None, None
            try:
                # 从路径加载原始、全分辨率的图像
                ...
                gr.Error(f"从路径加载Gemini Edit图片失败: {e}")
                return None, None
        
        gemini_hidden_receiver.change(
            fn=on_image_path_received,
            inputs=[gemini_hidden_receiver],
            outputs=[gemini_image_edit_state, visible_display_img]
        )
        ...
        submit_btn.click(
            fn=on_submit,
            inputs=[prompt_input, gemini_image_edit_state],
