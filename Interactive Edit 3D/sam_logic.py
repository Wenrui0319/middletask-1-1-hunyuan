...
def create_sam_ui(sam_predictor_global, device):
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=2):
            transparent_placeholder = np.zeros((1, 1, 4), dtype=np.uint8)
            interactive_display = gr.Image(label="工作区 (从左侧文件浏览器点击'编辑'发送图片)", type="numpy", interactive=True, height=700, sources=[], value=transparent_placeholder, elem_id="sam_interactive_display")
            with gr.Group():
