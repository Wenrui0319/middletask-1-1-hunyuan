...
核心技术路线为“**图像编辑 + 条件化3D生成**”。用户通过Web界面上传图片，然后可以使用多种方式进行编辑（交互式实例分割、角色的身体和服饰分离、文本驱动的细粒度编辑），最后将结果图像作为关键条件，驱动混元3D模型生成高质量的3D资产。
## 2.开发方面工作
### 2.1.SAM2 Demo分析和测试
...
SAM2部署效果图，原生支持点选和框选
![img_v3_02pc_e6c3812a-3eeb-4f5f-a564-2682b621e5eg](https://github.com/user-attachments/assets/8078290f-05aa-4be1-bc0e-57f1c454e53e)

### 2.2.混元3D模型 Gradio Demo分析和测试
部署效果图
> ![高质量动漫图分割效果](./assets/hunyuanDemo.png)
### 2.3.工作流和UI的整体设计
UI设计草图
> ![高质量动漫图分割效果](./assets/UIDraft.png)
### 2.4.SAM的Gradio Demo实现
sequence：
```mermaid
sequenceDiagram
    participant User as 用户
    participant GradioUI as Gradio 界面
    participant Backend as 后端函数
    participant SamPredictor as SAM 预测器模型

    %% 1. 应用加载和模型加载
    Note over GradioUI, Backend: 应用启动
    GradioUI->>Backend: application.load() 触发 load_model(默认模型)
    Backend->>SamPredictor: 创建 SamPredictor 实例
    SamPredictor-->>Backend: 返回预测器实例
    Backend-->>GradioUI: 将预测器存入 predictor_state

    %% 2. 用户上传图片
    User->>GradioUI: 上传图片 (input_image_upload)
    GradioUI->>Backend: 调用 set_image_for_predictor(predictor, image)
    Backend->>SamPredictor: 调用 predictor.set_image(image)
    SamPredictor-->>Backend: 内部状态已更新
    Backend-->>GradioUI: 更新/重置界面状态 (interactive_display, original_image_state, mask_state, history_state 等)

    %% 3. 交互式分割 (点选或框选)
    User->>GradioUI: 在图片上点击或画框 (interactive_display)
    GradioUI->>Backend: select() 触发 interactive_predict(...)
    Backend->>Backend: 根据用户输入更新 history_state
    Backend->>SamPredictor: 调用 predictor.predict(points, box, ...)
    SamPredictor-->>Backend: 返回蒙版(masks)和分数(scores)
    Backend->>Backend: 筛选最佳蒙版并调用 visualize_prompts()
    Backend-->>GradioUI: 返回更新后的图片、蒙版和历史状态
    GradioUI-->>User: 显示带有蒙版和提示点的图片

    %% 4. 抠出单个物体
    User->>GradioUI: 点击 "抠图" 按钮 (cut_out_btn)
    GradioUI->>Backend: click() 触发 single_cutout(image, mask)
    Backend->>Backend: 调用 cut_out_object(image, mask)
    Backend-->>GradioUI: 返回带透明背景的抠图
    GradioUI-->>User: 在 Gallery 中显示结果

    %% 5. 分割所有物体
    User->>GradioUI: 点击 "分割所有物体" 按钮 (cut_everything_btn)
    GradioUI->>Backend: click() 触发 generate_everything(predictor, image)
    Backend->>Backend: 初始化 SamAutomaticMaskGenerator
    Backend->>Backend: 调用 generator.generate() 获取所有蒙版
    Backend->>Backend: 循环调用 cut_out_object() 创建所有抠图
    Backend-->>GradioUI: 返回彩色蒙版图和所有抠图列表
    GradioUI-->>User: 更新主显示区并展示所有抠图

    %% 6. 重置流程
    User->>GradioUI: 点击 "重置" 按钮 (reset_btn)
    GradioUI->>Backend: click() 触发 reset_all(predictor, image)
    Backend->>SamPredictor: 再次调用 predictor.set_image(image) 以重置
    Backend-->>GradioUI: 返回所有组件的初始状态 (清空历史、蒙版等)
    GradioUI-->>User: 界面恢复到刚上传图片时的状态
```
演示视频：
...
> 图 4 分离人体和衣物的两阶段 
> ![角色提取&衣物和人体分离](./assets/suanfa.png)
1.  **第一阶段：角色提取**
    *   **输入**：用户上传的原始图片。
...
4. 创意性工作，例如生成不存在的部件、调整角色细节。
工作流展示：
> ![高质量动漫图分割效果](./assets/EdittingStream.png)
> ![高质量动漫图分割效果](./assets/EdittingStream2.png)
## 3.开发计划更新
### 3.1 任务分工
组长，主持日常工作：杜斌
汇报，外联：王雯睿
