...
- **Web Server**:
    ...
    - **技术栈建议**: Python (Flask/FastAPI) + JavaScript (Three.js/React)。
- **实例分割模型服务**:
...
4. 目前拟基于SAM2的demo进行二次开发，通过github协作。
## 1.5.拟交付成果
...
- **蓝图**: 参考SAM在线Demo ([https://segment-anything.com/demo](https://segment-anything.com/demo))
- **实施方案**:
    1.  **前端框架**: 选择React或Vue，便于组件化开发。
    2.  **图像交互模块**:
    ...
    4.  **后端通信**: 使用`axios`或`fetch` API与Web Server进行异步通信。
...
### 7.1 任务分工
主要分成模型组（2人）和平台组（2人）。模型组负责抹胸的调研、测试，系统的联合测试；平台组负责系统设计和开发。
