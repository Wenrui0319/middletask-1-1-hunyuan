...
// 70 MB default max image upload size
const MAX_FILE_SIZE_IN_MB = 70;
const MAX_IMAGE_UPLOAD_SIZE = MAX_FILE_SIZE_IN_MB * 1024 ** 2;
...
export default function useUploadImage({
  ...
  const [commit, isMutationInFlight] = useMutation<useUploadImageMutation>(
    graphql`
      mutation useUploadImageMutation($file: Upload!) {
    ...
    onDrop: (
      ...
      if (fileRejections.length > 0 && fileRejections[0].errors.length > 0) {
        ...
        if (code === 'file-too-large') {
          setError(
            `File too large. Try an image under ${MAX_FILE_SIZE_IN_MB} MB`,
          );
          return;
        }
      }
