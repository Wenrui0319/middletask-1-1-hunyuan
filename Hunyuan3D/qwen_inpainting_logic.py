...
async def run_generation(prompt, neg_prompt, input_img, sampler, scheduler, steps, cfg, denoise, shift, strength, use_mask):
    """
    通用生成函数，根据 use_mask 参数决定是执行 inpainting 还是 image editing。
    """
    try:
...
# --- 3. Gradio UI Layout ---
with gr.Blocks(theme=gr.themes.Base(), title="Qwen Inpainting with ComfyUI", analytics_enabled=False, css=".info-icon {display: flex; align-items: center; justify-content: center;}") as demo:
    gr.Markdown("## Qwen Inpainting with ComfyUI")
    ...
    with gr.Row(equal_height=True):
        with gr.Column(scale=1):
            ...
            with gr.Row():
                ...
                inpaint_btn = gr.Button("局部重绘 (Inpainting)", size="lg", scale=1)
    ...
    all_outputs = [status_text, output_img, comparison_img, edit_btn, inpaint_btn]

    # We create partial functions to pass the use_mask flag
    from functools import partial
    edit_logic_generator = partial(run_generation, use_mask=False)
    inpaint_logic_generator = partial(run_generation, use_mask=True)

    edit_btn.click(fn=edit_logic_generator, inputs=all_inputs, outputs=all_outputs)
    inpaint_btn.click(fn=inpaint_logic_generator, inputs=all_inputs, outputs=all_outputs)
